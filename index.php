...
### CONFIG OF BOT ###
#####################
define('DEBUG_FILE_NAME', 'index.php'); // if you need read debug log, you should write unique log name
define('CLIENT_ID', 'local.5be283ae241127.23328826'); // like 'app.67efrrt2990977.85678329' or 'local.57062d3061fc71.97850406' - This code should take in a partner's site, needed only if you want to write a message from Bot at any time without initialization by the user
define('CLIENT_SECRET', 'DYtQv6Kev1LirYSGNMlw5mfyIXDv1Z3imYaFo04sOLCtv9Ei35'); // like '8bb00435c88aaa3028a0d44320d60339' - TThis code should take in a partner's site, needed only if you want to write a message from Bot at any time without initialization by the user
#####################
...
else if ($_REQUEST['event'] == 'ONAPPINSTALL')
{
	...
	$result = restCommand('imbot.register', Array(
		'CODE' => 'itrbot',
		'TYPE' => 'O',
		'EVENT_MESSAGE_ADD' => 'http://restbot.com/itr.php',
		'EVENT_WELCOME_MESSAGE' => 'http://restbot.com/itr.php',
		'EVENT_BOT_DELETE' => 'http://restbot.com/itr.php',
		'OPENLINE' => 'Y',
		'PROPERTIES' => Array(
			'NAME' => 'ITR bot'.(count($appsConfig)+1),
			'WORK_POSITION' => "мой бот",
			'COLOR' => 'RED',
		)
	), $_REQUEST["auth"]);
	$botId = $result['result'];
...
function itrRun($portalId, $dialogId, $userId, $message = '')
{
	...
	$menu0 = new ItrMenu(0);
	$menu0->setText('Что вам надо?');
	$menu0->addItem(1, 'Первое', ItrItem::openMenu(1));
	$menu0->addItem(2, 'Второе', ItrItem::openMenu(2));
	$menu0->addItem(3, 'Ожидать оператора', ItrItem::sendText('Ожидать оператора', true));

	$menu1 = new ItrMenu(1);
	$menu1->setText('Вы перешли на первое?');
	$menu1->addItem(1, 'Да', ItrItem::sendText('Да, вы перешли на первое', true));
	$menu1->addItem(2, 'Нет', ItrItem::sendText('Нет, вы перешли на первое', true));
	
	$menu2 = new ItrMenu(2);
	$menu2->setText('Вы перешли на второе?');
	$menu2->addItem(1, 'Да', ItrItem::sendText('Да, вы перешли на второе', true));
	$menu2->addItem(2, 'Нет', ItrItem::sendText('Нет, вы перешли на второе', true));
...
function restAuth($auth)
{
	...
	$queryData = http_build_query($queryParams = array(
		...
		'client_id' => 	'local.5be283ae241127.23328826',
		'client_secret' => 'DYtQv6Kev1LirYSGNMlw5mfyIXDv1Z3imYaFo04sOLCtv9Ei35',
